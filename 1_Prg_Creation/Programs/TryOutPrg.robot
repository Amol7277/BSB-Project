...
Library     DateTime
#Resource        ../1_Resource/Resource_TryOut.robot
...
#Payment Plan Dates
    ...
    Select Tryout Program & Select Activity
    Complete 1 step of Program Settings         ${TO_Program_Name}${Current_Date_Time}
    Create Division 1   ${TO_Div1_Name}    ${Div_Price}   ${Div_regi_startdate}   ${Div_regi_closedate}   ${Div_SeasonStart}
    ...     ${Div_SeasonEnd}   ${OldDate}     ${YoungDate}

    Division 1 Advance setting      ${Tryout_Fee}    ${EarlyBird_DD}     ${EarlyBird_Date}   ${DL-Dis}   ${DL-Fee}   ${DL-MFee}
    ...     ${Deposit}      ${EMI-1 Date}   ${EMI-1}    ${installmentFee}
    ...     ${EMI-2 Date}   ${EMI-2}    ${EMI-3 Date}   ${EMI-3}

    Create Division 2      ${TO_Div2_Name}    ${Div_Price}   ${Div_regi_startdate}   ${Div_regi_closedate}   ${Div_SeasonStart}
    ...     ${Div_SeasonEnd}   ${OldDate}      ${YoungDate}
    Division 2 Advance setting      ${Tryout_Fee}   ${LateFee_DF}    ${LateFee_Date}    ${Deposit}
...
Select Tryout Program & Select Activity
    ...
    wait until element is visible               (//button/span[contains(text(), " Select")])[2]
    scroll element into view                    (//button/span[contains(text(), " Select")])[2]

    click element                               (//button/span[contains(text(), " Select")])[2]
...
Division 1 Advance setting
    [Arguments]     ${Tryout_Fee}    ${EarlyBird_DD}     ${EarlyBird_Date}   ${DL-Dis}   ${DL-Fee}   ${DL-MFee}
    ...     ${Deposit}      ${EMI-1 Date}   ${EMI-1}    ${installmentFee}
    ...     ${EMI-2 Date}   ${EMI-2}    ${EMI-3 Date}   ${EMI-3}
    set selenium implicit wait            20s
    set selenium speed                    0.4
...
#TO Fee
    wait until element is visible         //input[@formcontrolname="tryOutFeeAmountCtrl"]
    input text                            //input[@formcontrolname="tryOutFeeAmountCtrl"]       ${Tryout_Fee}
...
Division 2 Advance setting
    [Arguments]    ${Tryout_Fee}    ${LateFee_DF}    ${LateFee_Date}     ${DL-Dis}   ${DL-Fee}   ${DL-MFee}
    ...             ${Deposit}  ${EMI-1 Date}   ${EMI-1}    ${installmentFee}
    ...             ${EMI-2 Date}   ${EMI-2}    ${EMI-3 Date}   ${EMI-3}
    set selenium implicit wait            20s
    set selenium speed                    0.4
...
Go to the Confirmation note and add some note into it
    ...
    input text                       //*[@formcontrolname="tryoutFeeConfirmationEmail"]/div[2]/div    Tryout Order Confirmation Notes
    input text                       //*[@formcontrolname="playerAcceptanceEmail"]/div[2]/div         Tryout Acceptance Notes
    input text                       //*[@formcontrolname="playerRejectedEmail"]/div[2]/div           Tryout Dismissal Notes
    input text                       //*[@formcontrolname="playerAcceptanceContract"]/div[2]/div      Policies Contract
    input text                       //*[@formcontrolname='confirmationEmail']/div[2]/div             Division Order Confirmation Notes
